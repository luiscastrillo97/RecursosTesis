...
#define ARCHIVO ("/Nodo3.txt")            // Ruta del archivo donde se guardaran los datos
#define ID_NODO 3                         // Identificador del nodo sensor
#define LED_BUILTIN 2                     // GPIO del diodo LED para informar estado del sistema
#define RXD2 16                           // GPIO RX para el envio de datos
#define TXD2 17                           // GPIO TX para el envio de datos
...
String getData() {
  ...
  String mensaje = measurements();
  appendFile(SD, ARCHIVO, mensaje.c_str());
  //Serial.print("getData " + mensaje);
  digitalWrite(LED_BUILTIN, HIGH);
  return mensaje;
}

void sendData(String datos) {
  Serial.print(datos);
  Serial2.write(datos.c_str());
}
...
void setup() {
  pinMode(LED_BUILTIN, OUTPUT);     // Inicializando el GPIO del LED
